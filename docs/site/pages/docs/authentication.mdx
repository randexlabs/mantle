...
Mantle does not upload generic gameplay assets. Use [Asphalt](https://github.com/jackTabsCode/asphalt)
for gameplay asset synchronization instead. Media files declared as part of a Mantle-managed resource,
such as an experience icon, thumbnail, game pass icon, developer product icon, or badge icon, remain part
of that resource and use the permissions required by the corresponding Roblox operation.
